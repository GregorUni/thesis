...
static int macsec_add_txsa(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (tx_sa) {
		rtnl_unlock();
		printk("macsec_add_txsa fehler1\n");
		return -EBUSY;
	}
	...
	if (!tx_sa) {
		rtnl_unlock();
		printk("macsec_add_txsa fehler2\n");
		return -ENOMEM;
	}
	...
	if (err < 0) {
		kfree(tx_sa);
		rtnl_unlock();
		printk("macsec_add_txsa fehler3\n");
		return err;
	}
...
static int macsec_del_rxsa(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (IS_ERR(rx_sa)) {
		rtnl_unlock();
		printk("macsec_del_rxsa fehler1\n");
		return PTR_ERR(rx_sa);
	}

	if (rx_sa->active) {
		rtnl_unlock();
		printk("macsec_del_rxsa fehler2\n");
		return -EBUSY;
	}
				    ...
				    struct nlattr *data[])
{
	...
	if (data[IFLA_MACSEC_CIPHER_SUITE]) {
		...
		case MACSEC_CIPHER_ID_CHACHA_POLY_256:    //chacha
			secy->key_len = MACSEC_CHACHA_POLY_256_SAK_LEN;
