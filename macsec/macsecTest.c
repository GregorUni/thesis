...
static void free_rx_sc_rcu(struct rcu_head *head)
{
	...
	printk("free_rx_sc_rcu start");
	free_percpu(rx_sc->stats);
	kfree(rx_sc);
	printk("free_rx_sc_rcu ende");
}
...
{
	printk("macsec_rxsc_put start");
	if (atomic_dec_and_test(&sc->refcnt))
		call_rcu(&sc->rcu_head, free_rx_sc_rcu);
	printk("macsec_rxsc_put ende");
}

static void free_rxsa(struct rcu_head *head)
{
	struct macsec_rx_sa *sa = container_of(head, struct macsec_rx_sa, rcu);
	printk("free_rxsa start");
	crypto_free_aead(sa->key.tfm);
	free_percpu(sa->stats);
	kfree(sa);
	printk("free_rxsa ende");
}

static void macsec_rxsa_put(struct macsec_rx_sa *sa)
{
	printk("macsec_rxsa_put start");
	if (atomic_dec_and_test(&sa->refcnt))
		call_rcu(&sa->rcu, free_rxsa);

	printk("macsec_rxsa_put ende");
}
...
static void free_txsa(struct rcu_head *head)
{
	...
	printk("free_txsa start");
	crypto_free_aead(sa->key.tfm);
	free_percpu(sa->stats);
	kfree(sa);
	printk("free_txsa ende");
}

static void macsec_txsa_put(struct macsec_tx_sa *sa)
{
	printk("macsec_txsa_put start");
	if (atomic_dec_and_test(&sa->refcnt))
		call_rcu(&sa->rcu, free_txsa);

	printk("macsec_txsa_put ende");
}
...
static bool send_sci(const struct macsec_secy *secy)
{
	...
	printk("send_sci start");
	printk("send_sci ende");
...
static sci_t make_sci(u8 *addr, __be16 port)
{
	...
	sci_t sci;
	printk("make_sci start");
	memcpy(&sci, addr, ETH_ALEN);
	memcpy(((char *)&sci) + ETH_ALEN, &port, sizeof(port));
	printk("make_sci ende");
	return sci;
}
...
static sci_t macsec_frame_sci(struct macsec_eth_header *hdr, bool sci_present)
{
	...
	sci_t sci;
	printk("macsec_frame_sci start");
	if (sci_present)
		memcpy(&sci, hdr->secure_channel_id,
		       sizeof(hdr->secure_channel_id));
	else
		sci = make_sci(hdr->eth.h_source, MACSEC_PORT_ES);
	printk("macsec_frame_sci ende");
	return sci;
}
			       ...
			       bool sci_present)
{
	...
	printk("macsec_fill_sectag erfolgreich  \n");
...
static bool macsec_validate_skb(struct sk_buff *skb, u16 icv_len)
{
	...
	if (h->tci_an & MACSEC_TCI_VERSION){
		printk("macsec_validate_skb fehlgeschlagen1");
		return false;
	}
	...
	if ((h->tci_an & MACSEC_TCI_ES || h->tci_an & MACSEC_TCI_SCB) &&
	    (h->tci_an & MACSEC_TCI_SC))
	{
		printk("macsec_validate_skb fehlgeschlagen2");
		return false;
	}
	/* e) Bits 7 and 8 of octet 4 of the SecTAG are clear */
	if (h->unused)
	{
		printk("macsec_validate_skb fehlgeschlagen3");
		return false;
	}
	/* rx.pn != 0 (figure 10-5) */
	if (!h->packet_number)
	{
		printk("macsec_validate_skb fehlgeschlagen4");
		return false;
	}
	/* length check, f) g) h) i) */
	if (h->short_length)
	{
		printk("macsec_validate_skb erfolgreich1");
		return len == extra_len + h->short_length;
	}
	printk("macsec_validate_skb erfolgreich2");
					     ...
					     int num_frags)
{
	...
	*sg = (struct scatterlist *)(tmp + sg_offset);
	...
	printk("aead_request  *macsec_alloc_req erfolgreich");
	return req;
				      ...
				      struct net_device *dev)
{
	...
	if (!tx_sa) {
		secy->operational = false;
		kfree_skb(skb);
		printk("macsec_encrypt fehlgeschlagen1");
	...
	if (unlikely(skb_headroom(skb) < MACSEC_NEEDED_HEADROOM ||
		...
		if (likely(nskb)) {
			...
			kfree_skb(skb);
			printk("macsec_encrypt fehlgeschlagen2");
		...
		if (!skb) {
			macsec_txsa_put(tx_sa);
			printk("macsec_encrypt fehlgeschlagen3");
			return ERR_PTR(-ENOMEM);
		}
	}
	...
	if (pn == 0) {
		macsec_txsa_put(tx_sa);
		kfree_skb(skb);
		printk("macsec_encrypt fehlgeschlagen4");
	...
	if (skb->len - ETH_HLEN > macsec_priv(dev)->real_dev->mtu) {
		...
		macsec_txsa_put(tx_sa);
		kfree_skb(skb);
		printk("macsec_encrypt fehlgeschlagen5");
		return ERR_PTR(-EINVAL);
	}
	...
	if (unlikely(ret < 0)) {
		macsec_txsa_put(tx_sa);
		kfree_skb(skb);
		printk("macsec_encrypt fehlgeschlagen6");
		return ERR_PTR(ret);
	}
		printk("key. tfm: %p", tx_sa->key.tfm);
		printk("&iv: %p", (void*)iv);
		printk("&sg: %p", (void *)sg);
		printk("ret: %d", ret);
	...
	if (!req) {
		macsec_txsa_put(tx_sa);
		kfree_skb(skb);
		printk("macsec_encrypt fehlgeschlagen7");
		return ERR_PTR(-ENOMEM);
	}
	printk("iv vor req: %p",iv);
	printk("req inhalt: %p", (void*) req);
	...
	if (unlikely(ret < 0)) {
		...
		macsec_txsa_put(tx_sa);
		kfree_skb(skb);
		printk("macsec_encrypt fehlgeschlagen8");
		return ERR_PTR(ret);
	}
	...
	if (tx_sc->encrypt) {
		int len = skb->len - macsec_hdr_len(sci_present) -
			  secy->icv_len;
		printk("Hier ist die länge: %d", len);
	...
	//ret = crypto_rfc4106_encrypt(req);

	printk("%d", ret);
	if (ret == -EINPROGRESS) {
		printk("macsec_encrypt fehlgeschlagen9");
		return ERR_PTR(ret);
	} else if (ret != 0) {
		dev_put(skb->dev);
		kfree_skb(skb);
		aead_request_free(req);
		macsec_txsa_put(tx_sa);
		printk("macsec_encrypt fehlgeschlagen10");
		return ERR_PTR(-EINVAL);
	}
...
static bool macsec_post_decrypt(struct sk_buff *skb, struct macsec_secy *secy, u32 pn)
{
	...
	if (secy->replay_protect && pn < lowest_pn) {
		...
		printk("macsec_post_decrypt fehlgeschlagen1");
		return false;
	}
...
static void macsec_reset_skb(struct sk_buff *skb, struct net_device *dev)
{
	...
	printk("macsec_resest_skb erfolgreich");
}
				      ...
				      struct macsec_secy *secy)
{
	...
	printk("sk_buff *macsec_decrypt start");
	macsec_skb_cb(skb)->valid = false;
	skb = skb_share_check(skb, GFP_ATOMIC);
	if(!skb){
		printk("sk_buff *macsec_decrypt error1");
		return ERR_PTR(-ENOMEM);
	}
	...
	if (unlikely(ret < 0)) {
		kfree_skb(skb);
		printk("sk_buff *macsec_decrypt error2");
		return ERR_PTR(ret);
	}
	req = macsec_alloc_req(rx_sa->key.tfm, &iv, &sg, ret);
	if (!req) {
		kfree_skb(skb);
		printk("sk_buff *macsec_decrypt error3");
		return ERR_PTR(-ENOMEM);
	}
	...
	aead_request_free(req);
	printk("sk_buff macsec_decrypt ende");
	return skb;
}
...
static struct crypto_aead *macsec_alloc_tfm(char *key, int key_len, int icv_len)
{
	...
	if (IS_ERR(tfm)){
		...
		return tfm;}
	printk("key_len: %d", key_len);
	printk("icv_len: %d", icv_len);
	ret = crypto_aead_setkey(tfm, key, key_len);
	if (ret < 0){
		printk("crypto_aead *macsec_alloc_tfm fehlgeschlagen2");
		goto fail;}

	ret = crypto_aead_setauthsize(tfm, icv_len);
	if (ret < 0){
		printk("crypto_aead *macsec_alloc_tfm fehlgeschlagen3");
		goto fail;}

		printk("crypto_aead *macsec_alloc_tfm erfolgreich");
	return tfm;
fail:
...
static bool validate_add_rxsa(struct nlattr **attrs)
{
	...
	if (nla_len(attrs[MACSEC_SA_ATTR_KEYID]) != MACSEC_KEYID_LEN)
		return false;
	printk("validate_add_rxsa erfolgreich");
	return true;
}
...
static int macsec_add_txsa(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!attrs[MACSEC_ATTR_IFINDEX]){
		printk("macsec_add_txsa fehlgeschlagen1");
		return -EINVAL;}

	if (parse_sa_config(attrs, tb_sa)){
		printk("macsec_add_txsa fehlgeschlagen2");
		return -EINVAL;}
	printk("ich war hier1");
	if (!validate_add_txsa(tb_sa)){
		printk("macsec_add_txsa fehlgeschlagen3");
		return -EINVAL;}
	printk("ich war hier2");
	rtnl_lock();
	dev = get_dev_from_nl(genl_info_net(info), attrs);
	if (IS_ERR(dev)) {
		rtnl_unlock();
		printk("macsec_add_txsa fehlgeschlagen4");
		return PTR_ERR(dev);
	}
	printk("ich war hier3");
	secy = &macsec_priv(dev)->secy;
	tx_sc = &secy->tx_sc;

	assoc_num = nla_get_u8(tb_sa[MACSEC_SA_ATTR_AN]);
///xxx key_length up to 20 cause rfc4106(aes(gcm))
	/*if (nla_len(tb_sa[MACSEC_SA_ATTR_KEY]) != secy->key_len || secy->key_len != 20) {
		pr_notice("macsec: nl: add_txsa: bad key length: %d != %d\n",
			  nla_len(tb_sa[MACSEC_SA_ATTR_KEY]), secy->key_len);
		rtnl_unlock();
		printk("macsec_add_txsa fehlgeschlagen5");
		return -EINVAL;
	}
	*/
	printk("ich war hier4");
	tx_sa = rtnl_dereference(tx_sc->sa[assoc_num]);
	if (tx_sa) {
		printk("macsec_add_txsa fehlgeschlagen6");
		rtnl_unlock();
		return -EBUSY;
	}
	printk("ich war hier5");
	tx_sa = kmalloc(sizeof(*tx_sa), GFP_KERNEL);
	if (!tx_sa) {
		rtnl_unlock();
		printk("macsec_add_txsa fehlgeschlagen7");
		return -ENOMEM;
	}
	printk("ich war hier6");
	//printk(&tx_sa);
	//printk(secy->key_len);
	...
	if (err < 0) {
		kfree(tx_sa);
		rtnl_unlock();
		printk("macsec_add_txsa fehlgeschlagen8");
		return err;
	}
