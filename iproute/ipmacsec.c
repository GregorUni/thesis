...
//ccm
#define CCM_CIPHER_NAME "CCM-AES-128"

static const char *cs_id_to_name(__u64 cid)
{
	switch (cid) {
	case MACSEC_DEFAULT_CIPHER_ALT:
		return CCM_CIPHER_NAME; //ccm
...
static void usage(FILE *f)
{
	...
	fprintf(f,
		...
		"                  [ cipher { default | gcm-aes-128 | ccm-aes-128} ]\n"
		"                  [ icvlen { 8..16 } ]\n"
		"                  [ encrypt { on | off } ]\n"
		"                  [ send_sci { on | off } ]\n"
		"                  [ end_station { on | off } ]\n"
			    ...
			    struct nlmsghdr *n)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "cipher") == 0) {
			...
			//ccm
			else if (strcmp(*argv, "CCM-AES-128") == 0 ||
			    strcmp(*argv, "ccm-aes-128") == 0)
				cipher.id = MACSEC_DEFAULT_CIPHER_ALT;
			else
				invarg("expected: default, gcm-aes-128, ccm-aes-128",
				       *argv);
